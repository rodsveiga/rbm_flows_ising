...
class IsingMC():
    ...
    def getEnergy(self):
        ...
        currEnergy = 0
        for i in range(self.N_spins):
            currEnergy += -( self.spins[i]*self.spins[neighbours[i,0]] + 
                            self.spins[i]*self.spins[neighbours[i,1]] )
    ...
    def sweep(self):
        neighbours = self.neighbours()
        # One sweep (N_spins local updates)
        for i in range(self.N_spins):
            ...
            for j in range(4):
                deltaE += 2 * self.spins[site] * self.spins[neighbours[site,j]]
    ...
    def plot_thermodynamics(self, 
        ...
        plt.suptitle('%d x %d Ising model' %(self.L, self.L), fontsize=18)
        plt.show()
